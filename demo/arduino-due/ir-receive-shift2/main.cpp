   ...
   static message receive(){
      for(;;){
         ...
         for( uint_fast8_t i = 0; i < 32; ++i ){ 
            if( ! receive_pulse( 1, d, 300, 800 ) ){ continue; }
            if( ! receive_pulse( 0, d, 300, 2'000 ) ){ continue; }
            shift( msg.address, msg.address_inverted );
            shift( msg.address_inverted, msg.command );
            shift( msg.command, msg.command_inverted ); 
            shift( msg.command_inverted, ( d > 1'000 ) ? 0xFF : 00 );
         }
         return msg;         
      }        
   }
