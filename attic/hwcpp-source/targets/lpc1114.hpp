...
template< int ram_size, int rom_size, typename clock_frequency >
class lpc1114_base 
...
protected:
   ...
   static void HWCPP_INLINE configure_as_gpio(){
   ...
   static unsigned int pin_ad_start(){
   ...
   template< int channel >
   static bool pin_ad_would_block(){
   ...
   static unsigned int pin_ad_read(){
      ...
      while( pin_ad_would_block< channel >() ){}
   ...
   */
      ...
      public pin_pullup 
   {
      ...
      void HWCPP_INLINE direction_set_input() override final {
         lpc1114_base::direction_set_input( port, pin );
      }
    
      void HWCPP_INLINE direction_set_output() override final {
         lpc1114_base::direction_set_output( port, pin );
      }
      
      void HWCPP_INLINE set( bool x ) override final {
         lpc1114_base::set< port, pin >( x );
      }     
      
      bool HWCPP_INLINE get() override final {
         return lpc1114_base::get< port, pin >();
      }       
      
      void HWCPP_INLINE pullup_enable() override final {
         lpc1114_base::pullup_configure< port, pin, 1 >();
      }
   
      void HWCPP_INLINE pullup_disable() override final {
      ...
      public pin_ad< 10 >
   {
      ...
      void ad_start() override final {
         pin_ad_start< channel >();
      }
   
      bool ad_get_will_block() override final {
         return pin_ad_would_block< channel >();
      }
   
      pin_ad< 10 >::ad_value_type ad_get() override final {
   ...
   static void initialize_clock(){
      static_assert( 
            ( clock_frequency() == MHz_literal< 12 >() ) 
            || ( clock_frequency() == MHz_literal< 48 >() ), 
         "lpc1114 : unsupported clock frequency" 
      );
      
      if( clock_frequency() == MHz_literal< 12 >() ){
         ...
         if( clock_frequency() == MHz_literal< 48 >() ){
   ...
   static void __attribute__((noinline)) wait_4_cycles( int x ){
      asm volatile( 
            "   .align 2         \t\n"
            "1: sub %[reg], #1   \t\n" 
            "   bgt 1b           \t\n" 
   ...
   static void __attribute__((always_inline)) wait_busy( const duration d ){
      const auto ns = d.ns();
      ...
      // wait_4_cycles( ( clock_frequency().MHz() / 3 ) * d.us() );
      // wait_4_cycles( 4 * d.us() );
      // wait_4_cycles( 1'000'000 );
   }      
